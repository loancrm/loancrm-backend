...
// Get all active accounts with id and email
function getAllActiveAccounts() {
    return new Promise((resolve, reject) => {
        dbConnect.query(`SELECT accountId, emailId FROM accounts WHERE status = 1`, (err, results) => {
...
// Get active user IDs by accountId
function getActiveSourcedByIdsByAccount(accountId) {
    console.log(accountId)
    return new Promise((resolve, reject) => {
        dbConnect.query(
            `SELECT id FROM users WHERE userType = 3 AND status = 'Active' AND accountId = ?`,
            [accountId],
            (err, result) => {
                if (err) return reject(err);
...
// Get joining date
function getJoiningDate(userId) {
    return new Promise((resolve, reject) => {
        dbConnect.query(
            `SELECT joiningDate FROM users WHERE id = ?`,
            [userId],
            (err, result) => {
                if (err) return reject(err);
                if (result.length === 0) return resolve('N/A');
                resolve(moment(result[0].joiningDate).format('MMM DD, YYYY'));
            }
        );
    });
}

async function getLeadsAndCallbacksCountForActiveSources(accountId, activeSourcedByIds) {
    try {
        ...
        // if (activeSourcedByIds.length === 0) return [];
...
`;
        const [leadsCounts, loanLeadsCounts, callbacksCounts, leadsCountsThisMonth, loanleadsCountsThisMonth, callbacksCountsThisMonth, docsToday, docsThisMonth] = await Promise.all([
            ...
            new Promise((resolve, reject) => {
                dbConnect.query(
                    `SELECT leadId FROM leaddocuments WHERE createdOn >= ? AND createdOn < ? AND accountId = ?`,
                    [today, tomorrow, accountId],
            ...
            new Promise((resolve, reject) => {
                dbConnect.query(
                    `SELECT leadId FROM leaddocuments WHERE createdOn >= ? AND createdOn < ? AND accountId = ?`,
                    [firstDayOfMonth, nextMonth, accountId],
                    (err, result) => (err ? reject(err) : resolve(result))
                );
            })
        ]);
...
const getSourceName = async (accountId, userId) => {
    try {
        return new Promise((resolve, reject) => {
            const sql = `SELECT name FROM users WHERE id = ? AND accountId = ? AND userType = 3 AND status = 'Active' LIMIT 1`;
            dbConnect.query(sql, [userId, accountId], (err, result) => {
                if (err) {
                    console.error("DB error in getSourceName:", err);
                    return reject(err);
                }
                if (result.length === 0) return resolve(""); // No matching user
                resolve(result[0].name);
            });
        });
    } catch (error) {
        console.error("Error in getSourceName:", error);
        return "";
    }
};
async function sendLeadsReport(account) {
    try {
        const sourcedByIds = await getActiveSourcedByIdsByAccount(account.accountId);
        if (!sourcedByIds || sourcedByIds.length === 0) {
            console.log(`No active sourcedBy IDs found for accountId: ${account.accountId}. Skipping email.`);
            return;
        }
        const counts = await getLeadsAndCallbacksCountForActiveSources(account.accountId, sourcedByIds);
        ...
        const countsHTML = await Promise.all(counts.map(async (item, index) => {
            ...
            const SourcedByName = await getSourceName(account.accountId, item.sourcedBy);
            const joiningDate = await getJoiningDate(item.sourcedBy);
            return `
                <tr>
...
async function sendAllAccountsReport() {
    const accounts = await getAllActiveAccounts();
    for (const account of accounts) {
        await sendLeadsReport(account);
    }
}
